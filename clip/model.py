...
class ResidualAttentionBlock_BDC_CLIP(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        if self.attn_mask is None  and (self.layer_index in [3, 7, 11]):
            attn_output = attn_output.reshape(attn_output.size()[0]//self.frames, self.frames,  *attn_output.size()[1:])
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, text_layer=False, design_details=None):
        ...
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock_BDC_CLIP(width, heads, attn_mask, text_layer, i, design_details) for i in range(layers)])
